...
**The AWS account exists**: `<ACCOUNT_ID_REDACTED>`, region `eu-west-3`, namespace `cb-ec11b3c2`,
...
Environment, already verified:
  account   <ACCOUNT_ID_REDACTED>   region eu-west-3   namespace cb-ec11b3c2
...
3. Verify no sensitive value exists in the repository OR ITS GIT HISTORY: account IDs (in
   particular <ACCOUNT_ID_REDACTED>), ARNs, key-shaped strings, hostnames, session names. A working-tree
