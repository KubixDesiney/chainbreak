...
discussions, and commit messages on this repository — and also applies when an individual
is officially representing the community in public spaces.

## Enforcement

CHAINBREAK currently has a single maintainer and no separate conduct-response team, so this
project reuses the one private channel it has rather than inventing a contact address that
doesn't exist. Report violations through **GitHub's Private Vulnerability Reporting** on
this repository (Security tab → "Report a vulnerability") — the same route
[SECURITY.md](SECURITY.md) uses for security reports. It reaches the maintainer privately
and is not visible to the public. Note in your report that it is a conduct matter, not a
vulnerability, so it isn't triaged as one.
